...
import {
  Avatar, Button, Card, Divider, Table, Tooltip,
} from 'antd';
...
export default () => {
  ...
  const {
    id: clusterID, name: clusterName, fullPath: clusterFullPath, type, parentID,
  ...
  const { run: refreshCluster } = useRequest(() => getCluster(clusterID).then(({ data: result }) => {
    setCluster(result);
    ...
    <Detail>
      <div>
        <div className={styles.avatarBlock}>
          <Avatar
            className={`${styles.avatar} identicon bg${utils.getAvatarColorIndex(clusterName)}`}
            size={64}
            shape="square"
          >
