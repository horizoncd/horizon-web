...
// @ts-ignore
const MonitorSearchForm = ({formData, pods, containers, dashboard, baseUrl}) => {
    ...
    return <Form.Item label="Pods" name="podName">
      <Select style={{width: 400}} mode="multiple">
        <Select.Option key="all" value="all" style={{textAlign: 'right', paddingRight: '5px'}} disabled>
          <Checkbox checked={history.location.query?.podName?.length === pods.length}
                    onChange={(e) => selectAllPods(e.target.checked)}>全选</Checkbox>
        </Select.Option>
        {
  ...
  const formatContainerSelect = () => {
    return <Form.Item label="Container" name="container">
  ...
  return (
    <div>
      <Form
        ...
        {
          (dashboard === 'container') && pods.length > 0 && <Button type={"primary"} onClick={() => {
            setVisible(true);
          }}>
            多容器对比
          </Button>
        ...
        >
          <div>
            <div>
              ...
              <Select style={{width: 400}} mode="multiple" onChange={(value, options) => {
              ...
              <div style={{marginTop: 10}}>
                ...
                {
                  rangeType === 'custom' && <DatePicker.RangePicker
                    showTime
                    style={{marginTop: 10}}
