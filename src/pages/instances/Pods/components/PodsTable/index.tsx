...
export default (props: { data: CLUSTER.PodInTable[], cluster?: CLUSTER.Cluster | CLUSTER.ClusterV2, noMicroApp?: boolean }) => {
  ...
  const formatConsoleURL = (p: CLUSTER.PodInTable) => {
    ...
    return `/instances${fullPath}/-/webconsole?namespace=${p.namespace}&podName=${p.podName}&containerName=${p.containerName}&environment=${environment}`;
  };
  ...
  const formatPodMonitorURL = (p: CLUSTER.PodInTable) => `/instances${fullPath}/-/monitoring?monitor=horizon-pod&var-pod=${p.podName}`;

  const formatContainerMonitorURL = (podName: string, container: string) => `/instances${fullPath}/-/monitoring?monitor=horizon-container&var-pod=${podName}&var-container=${container}`;
